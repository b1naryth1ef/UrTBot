...
def loadConfig():
    ...
    cfg = ConfigFile("server.cfg", path=['config'], default=default_config)
    serverexe = cfg.command
    serversocket = cfg.usockname

def launch():
    global proc, procfile
    clients = []

    proc = subprocess.Popen(serverexe,  shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
